...
describe LanguagePack::VirgoWeb, type: :with_temp_dir do
  ...
  attr_reader :tmpdir, :virgo_web_pack

  let(:appdir) { File.join(tmpdir, "app") }

  before do
    @virgo_web_pack = LanguagePack::VirgoWeb.new(appdir)
    # TODO pass in Mock
    @virgo_web_pack.stub(:install_java)

    Dir.chdir(tmpdir) do
      Dir.mkdir("app")
      Dir.chdir(appdir) do
        Dir.mkdir("META-INF")
        virgo_web_pack.stub(:download_virgo) do
          FileUtils.copy( File.expand_path("../../support/fake-virgo.tar.gz", __FILE__), ".virgo/virgo.tar.gz")
        end
        virgo_web_pack.stub(:install_database_drivers)
      end
    end
  end
  ...
  describe "compile" do
    ...
    it "should create a .profile.d with connector port and heap size in JAVA_OPTS" do
      virgo_web_pack.stub(:install_virgo)
      virgo_web_pack.compile
